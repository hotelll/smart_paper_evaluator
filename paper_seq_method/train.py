...
# Model choices
tf.flags.DEFINE_string('clf', 'lstm',
                       "Type of classifiers. Default: cnn. You have four choices: [cnn, lstm, blstm, clstm]")

# Data parameters
tf.flags.DEFINE_string('data_file', './data/paper_seq2000.csv', 'Data file path')
...
tf.flags.DEFINE_integer('max_length', 30, 'Max document length')
tf.flags.DEFINE_integer('vocab_size', 3, 'Vocabulary size')
...
tf.flags.DEFINE_integer('decay_steps', 100000000, 'Learning rate decay steps')  # Learning rate decay
...
with tf.Graph().as_default():
    with tf.Session() as sess:
        ...
        def run_step(input_data, is_training=True):
            ...
            pred = vars['predictions']
            y_labels = vars['gt_labels']
            f1_score = metrics.f1_score(y_labels, pred)
        ...
        for train_input in train_data:
            ...
            if current_step % FLAGS.evaluate_every_steps == 0:
                print('\nValidation')
                acc, f1_score = run_step((x_valid, y_valid, valid_lengths), is_training=False)
                print("accuracy: {}    f1-score: {}".format(acc, f1_score))
